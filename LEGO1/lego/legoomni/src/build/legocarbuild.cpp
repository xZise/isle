...
LegoCarBuild::LookupTableActions LegoCarBuild::g_actorScripts[] = {
	{DunecarScript::c_igs001d3_RunAnim, // introduction
	 DunecarScript::c_igs002d3_RunAnim, // really leave?
	 DunecarScript::c_igs003d3_RunAnim, // finished
	 DunecarScript::c_igs004d3_RunAnim, // "come in, your car is waiting to be build"
	 DunecarScript::c_igs005d3_RunAnim, // "come in, build your car again"
	 DunecarScript::c_igs004d3_RunAnim, // "come in, your car is waiting to be build"
	 DunecarScript::c_igsxx1d3_RunAnim}, // other introduction
	{JetskiScript::c_ijs001d4_RunAnim, // introduction
	 JetskiScript::c_ijs003d4_RunAnim, // really leave?
	 JetskiScript::c_ijs004d4_RunAnim, // finished
...
MxResult LegoCarBuild::Create(MxDSAction& p_dsAction)
{
	...
	if (!result) {
		...
		m_finishedBuildOnCreate = m_buildState->m_finishedBuild;
...
void LegoCarBuild::ButtonUpOnClickState4()
{
	if (m_selectedPart) {
		VTable0x6c();
		m_animPresenter->SetShelfState(LegoCarBuildAnimPresenter::e_selected);
...
{
	m_unk0x178 = m_maybeOriginalSelectedPartTransform;
...
MxLong LegoCarBuild::Notify(MxParam& p_param)
{
	...
	if (m_worldStarted) {
		...
		case c_notificationButtonUp:
			...
			if (result || m_lastActorScript || m_buildState->m_animationState == LegoVehicleBuildState::e_finishedBuild || m_buildState->m_animationState == LegoVehicleBuildState::e_exiting) {
				m_missclickCounter = 0;
				break;
			}
		...
		case c_notificationButtonDown:
			assert(m_buildState);
			if (((m_buildState->m_animationState != LegoVehicleBuildState::e_finishedBuild) && (m_buildState->m_animationState != LegoVehicleBuildState::e_exiting)) &&
				(m_buildState->m_animationState != LegoVehicleBuildState::e_playingMovie)) {
...
undefined4 LegoCarBuild::HandleKeyPress(LegoEventNotificationParam* p_param)
{
	if (p_param->GetKey() == ' ' && m_buildState->m_animationState != LegoVehicleBuildState::e_finishedBuild && m_buildState->m_animationState != LegoVehicleBuildState::e_playingMovie) {
		if (m_numAnimsRun > 0) {
...
undefined4 LegoCarBuild::HandleEndAction(MxActionNotificationParam* p_param)
{
	MxS32 result = 0;
...
undefined4 LegoCarBuild::SelectPartFromMousePosition(MxLong p_x, MxLong p_y)
{
	...
	if (m_clickState == 5 && m_animPresenter->PartIsPlaced(m_selectedPart->GetName())) {
...
undefined4 LegoCarBuild::HandleButtonUp(MxLong p_x, MxLong p_y)
{
	undefined4 result = 0;
...
MxS32 LegoCarBuild::HandleMouseMove(MxLong p_x, MxLong p_y)
{
	MxS32 result = 0;
...
undefined4 LegoCarBuild::HandleControl(MxParam* p_param)
{
	undefined4 result = 0;
...
undefined4 LegoCarBuild::HandleType0Notification(MxNotificationParam* p_param)
{
	LegoEntity* entity;
	assert(m_buildState);
	...
	case LegoVehicleBuildState::AnimationState::e_finishedBuild:
		...
		break;
	case LegoVehicleBuildState::AnimationState::e_playingMovie:
		MxU32 jukeboxScript;
...
void LegoCarBuild::SetPartColor(MxS32 p_objectId)
{
	...
	m_selectedPart->SetColorNamed2(color);
...
{
	m_maybeOriginalSelectedPartTransform2 = m_selectedPart->GetLocal2World();
	m_maybeOriginalSelectedPartTransform = m_maybeOriginalSelectedPartTransform2;

	Vector3 lastColumnOfUnk0x1c0(m_maybeOriginalSelectedPartTransform[3]);
	...
	*unk0x178 = m_maybeOriginalSelectedPartTransform2;
	...
	if (m_animPresenter->PartIsPlaced(m_selectedPart->GetName())) {
		...
		if (!m_selectedPartIsPlaced) {
			...
			m_unk0x208 = m_maybeOriginalSelectedPartTransform2;
			m_unk0x2a4[0] += m_maybeOriginalSelectedPartTransform[3][0] - m_unk0x208[3][0];
			m_unk0x2a4[1] += m_maybeOriginalSelectedPartTransform[3][1] - m_unk0x208[3][1];
			m_unk0x2a4[2] += m_maybeOriginalSelectedPartTransform[3][2] - m_unk0x208[3][2];
		}

		*unk0x178 = m_maybeOriginalSelectedPartTransform;
	}
	else {
		...
		m_unk0x2a4[2] += (m_maybeOriginalSelectedPartTransform[3][2] - m_maybeOriginalSelectedPartTransform2[3][2]);
		m_unk0x178[3][2] = m_maybeOriginalSelectedPartTransform[3][2];
	}
}
...
void LegoCarBuild::StartActorScriptByType(undefined4 p_param)
{
	m_numAnimsRun++;
	m_lastActorScript = 0;
	MxS32 uVar6;
	...
	if (GameState()->GetCurrentAct() == LegoGameState::e_act2) {
		...
		switch (p_param) {
		case LookupTableActionType::e_0:
		case LookupTableActionType::e_1:
		case LookupTableActionType::e_2:
		case LookupTableActionType::e_3:
			switch (rand() % 3) {
			case 0:
		...
		case LookupTableActionType::e_shortExplanation:
			...
			uVar6 = m_lastActorScript;
			StopPlayingActorScript();

			if (uVar6 != DS_NOT_A_STREAM) {
				m_playingActorScript = uVar6;
				BackgroundAudioManager()->LowerVolume();
		...
		case LookupTableActionType::e_1:
			...
			break;
		case LookupTableActionType::e_2:
			m_lastActorScript = g_actorScripts[GetLookupIndex()].m_introduction3;
			StartActorScript(m_lastActorScript);
			...
			break;
		case LookupTableActionType::e_3:
			StartActorScript(g_actorScripts[GetLookupIndex()].m_introduction4);
			break;
		case LookupTableActionType::e_leaveUnfinished:
			StartActorScript(g_actorScripts[GetLookupIndex()].m_leaveUnfinished);
			break;
...
MxS32 LegoCarBuild::GetNextIntroduction()
{
	...
	case 1:
		return LookupTableActionType::e_1;
	case 2:
		return LookupTableActionType::e_2;
	case 3:
		return LookupTableActionType::e_3;
	default:
		return LookupTableActionType::e_0;
	}
}

// FUNCTION: LEGO1 0x10025db0
// FUNCTION: BETA10 0x1006ed18
void LegoCarBuild::TickleControl(const char* p_controlName, undefined4 p_time)
{
...
MxS32 LegoCarBuild::GetBuildMovieId(undefined4 p_carId)
{
	switch (p_carId) {
	case Helicopter_Actor:
